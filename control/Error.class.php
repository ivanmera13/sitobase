<?php
//controlamos el llamado a la base de datos configuramos el origen 
...
/**
*	Helper function implements singleton pattern (wouldnormally be inside the class)
*/
function &createError($qa_mail = null, $subject = null, $ask_permission = true)
{
	/**
	*	All PHP 5 Support static attribute inside class changed that to better OOP
...
/**
*	Error class that handle the error into a singleton pattern
*	(tricked with an outside function at the moment)
*
*	@package Error
*/
...
Class Error
{
	...
	*/
	...
	var $_qa_email = array('ivan.mera13@gmail.com');
	...
	*	@param string $error_msg Msg to display with Error
	...
	*/
	...
	function addError($level, $error_msg, $file, $function, $line)
	{
		# Backtrace
	...
	/**
	*	add_ctrl_mail: Allow to add an email to the one receiving error without duplicate
	*	
	...
	*/
	...
	function showError($level = ERROR_DEBUG)
	{
	...
	/**
	*	Allows of  logging of error to the database and/or sending of email to the diferent contact
	*	
	...
	*/
	...
	function logError($level = ERROR_DEBUG, $subject = null, $skipServerVarDump = false)
	{
		...
		{
			echo "${eol}_SERVER_ARRAY:		".$this->error_print_r_html($_SERVER, TRUE);
			echo "$eol$eol";
		}
	...
	*
	*	@param $increment_errors: If set to FALSE, it will only clean and send out a mail lost errors
	*	@access private
	*
	*/
	
	function getSendingPermision($increment_errors = TRUE)
	{
		...
		# if error are allowed to be sent
	...
	/**
	*	Print  Error: return error in format html
	*
	*	@param $data
	*	@param $return_data
	*	
	...
	*/
	...
	/**
	*	Register errors system of subscriptions 
	*	
	*	This function allow us to register in the table logs of subscriptions the errors found
	*	
	*	@param number $client id of client
	*	@param..
	*	@param string $error_message
	*	
	*	@return true if a error well register and false if not
	*	
	*	@access public
	*/
	# the funcion where all params
	...
	*/
	...
	function logSubscriptionError($error_message, $location = '')
	{
		$trace = debug_backtrace();
		if(empty($location))
		{
